function h = som_show(sMap, constraints, num_sliders, fig_no, sliders, curr_values, varargin)
...
if size(curr_values,1)>0
    current_values = curr_values;
end

% current_values = curr_values;

SA(1:n) = struct('axis_handle', NaN);
for i=1:n,                         % main loop
%   f = figure(fig_no); 
hold on;
  ...
  case 'comp'
    ...
    %     Newpos1 = [pos(1) pos(2) pos(3) 0.035];
        Newpos1=[pos(1) pos(2)-0.05 pos(3) 0.035];
    
        set(hslide(i),'position',Newpos1,'min',mn(i),'max',mx(i),'Value',current_values(i));
        callback_function = @(src, event) updateThreshold_final(src, event, sMap,fig_no, constraints{i}, mn, mx, current_values, constraints, sliders); 
        set(hslide(i), 'Callback', callback_function)
        %current_values = @(src, event) updateThreshold_final(src, event, sMap,fig_no, constraints{i}, mn, mx, current_values, constraints, sliders);

    
        SA(i).axis_handle = h_axes(i);

    end
    % thresholdValue = 0;
    % thresholdValue = get(hslide(i), 'Value');
    % 
    % disp([i, ': ', num2str(thresholdValue)]);
    % 
    % h_RoI = zeros(sMap.topol.msize(1) * sMap.topol.msize(2), 1);
    % h_RoI(sMap.codebook(:, end) > thresholdValue) = 1;
    % 
    % if i == 4 || i == 5 || i == 6
    % if i == 4
    %     x = 5;
    ...
    % end
    % 
    % som_show_add('hit', h_RoI, 'Markersize', 1, 'MarkerColor', 'none', 'EdgeColor', col, 'Subplot', [2, 3, 4, x]);
    % end
...
% for i = 1:n
%     S(i)=['set(gca,''xlim'',[' '0 ' 'get(gcbo,''value'')])'];
%     set(hslide(i),'callback',S(i));
% end
